...
void Player::enterState(PlayerState state)
{
	...
	case PlayerState::SUPERSHOOT:
		...
		assetManager->getSound(SoundName::SHIP_CHARGE).replay();
...
void Player::exitState(PlayerState state)
{
	...
	case PlayerState::SUPERSHOOT:
		...
		assetManager->getSound(SoundName::SHIP_SHOOT_UPGRADED).replay();
		...
		auto  bullet = std::make_shared<SuperBullet>
			(
				assetManager->getSprite(SpriteName::SHIP_BULLET_UPGRADED),
				assetManager->getSprite(SpriteName::SHIP_BULLET_UPGRADED_TRAIL),
...
void Player::updateShoot(float deltaTime)
{
	...
	auto  bullet = std::make_shared<Bullet>
		(
			assetManager->getSprite(SpriteName::SHIP_BULLET),
	...
	assetManager->getSound(SoundName::SHIP_SHOOT).replay();
}
...
void Player::updateSuperShoot(float deltaTime)
{
	...
	if (!Input::getButton("Shoot"))
	{
		...
		assetManager->getSound(SoundName::SHIP_CHARGE).stop();
	}

	chargeTimer -= deltaTime;

	if (chargeTimer <= 0.f) assetManager->getSound(SoundName::SHIP_CHARGE).stop();
}
